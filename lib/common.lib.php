...
function get_paging($write_pages, $cur_page, $total_page, $url, $add="")
{
    ...
    $str = '';
    if(!is_mobile()) { // pc 페이지네이션
        if ($cur_page > 1) {
            $str .= '<a href="'.$url.'1'.$add.'" class="pg_page pg_start"><img src="/theme/basic/img/ico_prev_double.svg" alt="처음"></a>'.PHP_EOL;
        }
        ...
        if ($cur_page > 4) $str .= '<a href="'.$url.($start_page-1).$add.'" class="pg_page pg_prev"><img src="/theme/basic/img/ico_prev_black.svg" alt="이전"></a>'.PHP_EOL;
        ...
        if($total_page > 4 && $cur_page < ($total_page - 4) && !is_mobile()) { // 1 2 3...마지막페이지
            $str .= '<span class="space">...</span>'.PHP_EOL;
            $str .= '<a href="'.$url.$total_page.$add.'" class="pg_page pg_next">'.$total_page.'</a>'.PHP_EOL;
            
        }
    
        if ($total_page> 4 && $cur_page < $total_page) $str .= '<a href="'.$url.($end_page+1).$add.'" class="pg_page pg_next"><img src="/theme/basic/img/ico_next_black.svg" alt="다음"></a>'.PHP_EOL;
    
        // if ($total_page > $end_page) $str .= '<a href="'.$url.($end_page+1).$add.'" class="pg_page pg_next">다음</a>'.PHP_EOL;
    
        if ($cur_page < $total_page) {
            $str .= '<a href="'.$url.$total_page.$add.'" class="pg_page pg_end"><img src="/theme/basic/img/ico_next_double.svg" alt="맨끝"></a>'.PHP_EOL;
        }
    } else { // 모바일
        if($total_page > 1) {
            if($cur_page > 1) { // previous
                $str .= '<a href="'.$url.($cur_page - 1).'" class="page-btn prev">Previous</a>';
            } else {
                $str .= '<a href="#" class="page-btn prev disable">Previous</a>';
            }
    
            if($cur_page < $total_page) { // next
                $str .= '<a href="'.$url.($cur_page + 1).'" class="page-btn next">Next</a>';
            } else {
                $str .= '<a href="#" class="page-btn next disable">Next</a>';
            }
        }
    }

    if ($str)
        return "<nav class=\"pg_wrap\"><h3 class='hidden'>리스트 페이지네이션</h3><span class=\"pg\">{$str}</span></nav>";
    else
        return "";
}
...
function get_sql_search($search_ca_name, $search_field, $search_text, $search_operator='and')
{
    ...
    if($search_ca_name && $bo_table == 'portfolio') {
