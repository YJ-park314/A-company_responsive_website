...
if ($board['bo_use_category']) {
    $is_category = true;
    $category_href = str_replace('/www/bbs', '/bbs', get_pretty_url($bo_table));
    ...
    if ($bo_table == "portfolio" || $bo_table == "estimate") {
        for ($i=0; $i<count($categories); $i++) {
            $category = trim($categories[$i]);
            if ($category=='') continue;

            $category_option .= '<li><a href="'.($category_href."&sfl=ca_name&stx=".urlencode($category)).'"'; 
