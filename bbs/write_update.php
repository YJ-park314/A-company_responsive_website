...
include_once(G5_SKIN_PATH.'/board/portfolio/write_update.head.skin.php');
...
if ($bo_table !== "portfolio" && $bo_table !== "estimate") {  //추가
    /* original */
...
for ($i = 1; $i <= 10; $i++) {
    $var_name = "wr_link{$i}"; // 변수명 동적으로 생성
    $$var_name = ''; // 기본값 설정

    if (isset($_POST[$var_name])) {
        $$var_name = substr($_POST[$var_name], 0, 1000);
        $$var_name = trim(strip_tags($$var_name));
    ...
    if($bo_table == 'portfolio') {
        $sql = " insert into $write_table
                    set wr_num = " . ($w == 'r' ? "'$wr_num'" : "(SELECT IFNULL(MIN(wr_num) - 1, -1) FROM $write_table as sq) ") . ",
                         ...
                         wr_9 = '$wr_9',
                         wr_10 = '$wr_10' ";
    } else { // 포트폴리오 코드 수정 이전에 만든 게시판은 wr_link 컬럼이 2까지 밖에 없음
        $sql = " insert into $write_table
                    set wr_num = " . ($w == 'r' ? "'$wr_num'" : "(SELECT IFNULL(MIN(wr_num) - 1, -1) FROM $write_table as sq) ") . ",
                         wr_reply = '$wr_reply',
                         wr_comment = 0,
...
if ($file_upload_msg)
    ...
    if($bo_table == 'portfolio' || $bo_table == 'estimate') { // 포트폴리오 게시판은 상세로 가지 않음
